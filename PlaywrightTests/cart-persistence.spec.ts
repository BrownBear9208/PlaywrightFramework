...
test.describe('Cart State Persistence', () => {
  ...
  test('TC-800 | Cart badge persists through product detail navigation', { tag: '@regression' }, async ({
    inventoryPage, itemDetailPage
  }) => {
  ...
  test('TC-801 | Cart badge persists across cart and inventory round-trip', { tag: '@regression' }, async ({
  ...
  test('TC-802 | Cart badge persists after external navigation to About page', { tag: '@regression' }, async ({
    page, inventoryPage
  }) => {
    await inventoryPage.addItemToCartByName('Sauce Labs Backpack');
    await expect(inventoryPage.cartBadgeLocator).toHaveText('1');
  ...
  test('TC-804 | Previously added items display Remove button after re-entering inventory', { tag: '@regression' }, async ({
