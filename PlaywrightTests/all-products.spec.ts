...
test.describe('Product Catalog - Full Coverage', () => {
  ...
  for (const product of ALL_PRODUCTS) {
    test(`TC-600 | Product detail page renders correctly for "${product}"`, { tag: '@regression' }, async ({
    ...
    test(`TC-601 | Add and remove "${product}" toggles cart state correctly`, { tag: '@regression' }, async ({
      inventoryPage
    }) => {
  ...
  test('TC-611 | Full catalog purchase completes with correct total calculation', { tag: '@regression' }, async ({
